...
interface ProductionHandoverDialogProps {
  orderId: string;
  onConfirm: (data: { prod_delivered_qty: number; prod_scrap_qty: number; prod_reported_by: string }) => Promise<void>;
  onCancel: () => void;
}

export function ProductionHandoverDialog({ orderId, onConfirm, onCancel }: ProductionHandoverDialogProps) {
  ...
  const handleConfirm = async () => {
    ...
    setApiError(null);
    try {
      await onConfirm({ prod_delivered_qty: delivered, prod_scrap_qty: scrap, prod_reported_by: reportedBy.trim() || 'current_user' });
    } catch (e: unknown) {
      ...
      <div className="bg-card border border-border rounded-xl shadow-elevated w-full max-w-md mx-4 animate-slide-in-right">
        ...
        <div className="flex items-center justify-between p-4 border-b border-border">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold">Production → Logistics Handover</h2>
          </div>
        ...
        <div className="p-4 space-y-4">
          <div className="flex items-center gap-3 text-sm bg-muted rounded-lg px-3 py-2">
            ...
            <span className="text-xs font-semibold text-success">Logistics</span>
          </div>
          ...
          {delivered > 0 && (
            <div className="text-xs text-muted-foreground bg-muted rounded-lg px-3 py-2">
              Finished qty: <strong className="text-foreground">{delivered - scrap}</strong>
            </div>
          )}
        </div>
